...
buffer and accessors to such buffer. In that case, these properties must be
combined as follows:
...
If no work-group size is specified when using local internalization for any of
the kernels involved in the fusion, no internalization will be performed.
