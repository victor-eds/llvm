...
event handler::finalize() {
  ...
  if (type == detail::CG::Kernel && !MQueue->is_in_fusion_mode() &&
    ...
    // the graph is not changed, and the queue is not in fusion mode, then this
    // faster path is used to submit kernel bypassing scheduler and avoiding
    // CommandGroup, Command objects creation.
